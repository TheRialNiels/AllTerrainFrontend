<template>
  ...
  const updateUni = async () => {
    ...
    try {
      const response = await toDoRequest.patch(`api/universidad/${form.id}/`,form);
      ...
      if (response.status === 200) {
        ...
        const updatedUniIndex = uniDataArray.findIndex((uni) => uni.id === form.id);
        if (updatedUniIndex !== -1) {
          uniDataArray[updatedUniIndex] = { ...uniDataArray[updatedUniIndex], ...form };
        }

<template>
  ...
  const editAdv = (cell) => {
    ...
    props.form.nombreCompleto= data.nombreCompleto;
    props.form.tipoAsesor = data.tipoAsesor;
    props.form.idEquipo = data.idEquipo;

    emit("change-is-editing");
  }
  
  const deleteAdv = async (cell) => {
    let res = await SwalDelete('No podrás revertir esto')
  
    if (res) {
      SwalCustomLoading('Eliminando asesor...')
  
      try {
        let data = cell.getRow().getData()
  
        let response = await toDoRequest.delete(`api/asesor/${data.id}/`)
  
        if (response.status === 204) {
          // Delete row
          cell.getRow().delete()
          SwalSuccess(' Asesor Eliminado')
        }
      } catch (error) {
        SwalError(error.response.data.error || 'Error al eliminar el asesor')
      }
    }
  }
  
  watch(() => props.advData, (newAdvData) => {
    if (tabulator.value) {
      tabulator.value.setData(newAdvData);
    }
  });
  
  onMounted(async () => {
    await props.getAdvData();
    initTable();
  })
  </script>

    ...
    columns: [
      ...
      },
      {
        ...
        formatter: (cell) => {
          ...
          return data.role === "admin" ? "Administrador" : data.role === "encargado" ? "Encargado" : "Juez";
        },
      },
      {
        title: "Editar",
...
const editUser = (cell) => {
  ...
  props.form.username = data.username;
  props.form.first_name = data.first_name;
  props.form.last_name = data.last_name;
  props.form.role = data.role;

  emit("change-is-editing");
};

const deleteUser = async (cell) => {
  let res = await SwalDelete("No podrás revertir esto");

  if (res) {
    SwalCustomLoading("Eliminando Usuario...");

    try {
      let data = cell.getRow().getData();

      let response = await toDoRequest.delete(
        `/api/auth/delete-user/${data.id}/`
      );

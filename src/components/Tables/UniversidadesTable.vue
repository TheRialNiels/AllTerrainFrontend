<template>
  ...
  const deleteUni = async (cell) => {
    let res = await SwalDelete('No podrás revertir esto')
  
    if (res) {
      SwalCustomLoading('Eliminando Universidad...')
  
      try {
        let data = cell.getRow().getData()
  
        let response = await toDoRequest.delete(`api/universidad/${data.id}/`)
  
        if (response.status === 204) {
          // Delete row
          cell.getRow().delete()
          SwalSuccess('Universidad Eliminado')
        }
      } catch (error) {
        SwalError(error.response.data.error || 'Error al eliminar la universidad')
      }
    }
  }
  
  watch(() => props.UniData, (newTeamData) => {
    if (tabulator.value) {
